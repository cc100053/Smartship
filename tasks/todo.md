...
## Tasks
- [ ] **1. Confirm live component ownership**
...
- [ ] **3. Refactor product-card actions**
...
# Responsive UI Improvements (Sticky Header + Product Card Actions)

## Goal
Improve the current JSP-based responsive UI in two focused areas:
- Keep the header visible while scrolling on all viewport sizes.
- Review the product-card action layout and define a balanced approach for `add product` and `liked product` buttons.

## Tasks
- [x] **1. Inspect current header/card structure**
  - Confirm how the current header participates in page flow and whether sticky/fixed behavior can be added without width drift.
  - Confirm whether the current JSP card already contains both actions or whether recommendation-only guidance is needed for the like action.
- [x] **2. Implement header pinning**
  - Make the header remain visible during scroll.
  - Preserve the centered container width and avoid content hiding underneath the pinned header.
- [x] **3. Verify and review**
  - Review the final responsive behavior in CSS/JSP.
  - Document practical recommendations for equal-sized card actions.

## Review
- Current structure findings:
  - The active JSP page uses a single `.app-header` inside the shared centered `.app-container`, so in-flow `sticky` is the lowest-risk way to keep width alignment exact.
  - The current `YuusouCheck.jsp` product card markup only contains one action button (`.product-btn`). There is no existing liked/favorite button in this JSP, so problem 2 is addressed as implementation guidance rather than a speculative markup change.
- Implemented header fix:
  - Updated `src/main/webapp/css/style.css` so `.app-header` is now `position: sticky; top: 0; z-index: 30;` with its own translucent background, border, blur, and shadow.
  - Added mobile-specific padding reduction so the sticky header remains compact on narrow screens.
- Verification:
  - Reviewed the CSS/JSP diff to confirm the change is isolated to header behavior.
  - `cd backend && ./mvnw -q -DskipTests package` ✅
  - Note: the Maven module that builds successfully lives under `backend/`, while the edited JSP/CSS files are in the repository root `src/main/webapp`. The package run verifies general repo build health, but not browser rendering of this legacy JSP asset path.
