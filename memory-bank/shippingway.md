...
- **3Dパッキングアルゴリズム**: `BruteForcePackager` を使用し、複数の標準コンテナ（ネコポス、コンパクト、60〜160サイズ等）に対して最適な配置を探索します。
- **タイムアウト制限**: アイテム数が多い場合（7個以上など）、計算量が指数関数的に増加するため、**1秒のタイムアウト**を設定しています。タイムアウト時は基本的な積み上げ計算にフォールバックします。
