...
## 概要
本システムでは、カート内の商品構成や手動入力されたサイズに基づき、利用可能な配送キャリア（配送方法）を評価し、最適なプランを提案します。
...
- **現在の動作 (Library-only モード)**:
  - `FastLargestAreaFitFirstPackager`（ライブラリの高速ヒューリスティック）を**そのまま使用**。
  - **独自のソート/比較/薄物スタッキング補正は無効化**（`USE_LIBRARY_ONLY = true`）。
  - コンテナは**フォールバック固定リスト**（ネコポス、コンパクト、60〜160サイズ等）から選択。
  - 失敗時は巨大コンテナを試し、最終的に**積み上げ計算**へフォールバック。
  - 1回のパッキングに**2秒**の期限を設定。

#### 追加の後処理 (Compaction)
ライブラリが作る配置に**隙間が残る**問題に対し、以下の後処理を適用：
- **コンパクション（隙間詰め）**を複数パス実行。
- 各パスで「**体積増加が最小になる移動**」を優先（= 体積の減少が最大の移動）。
- 同一の体積減少の場合は、**サイズ合計 → 形状の正方形度 → 最大辺 → 体積**の順に比較。
- 最後に座標を正規化し、**実際のバウンディングボックスに一致する寸法**を算出。

##### 切り替えメモ
- `PackingService` の `USE_LIBRARY_ONLY` を **false** にすると、従来の独自ロジック（複数ソート戦略・独自比較・薄物補正など）に戻せます。

#### 圧縮計算 (Compression)
柔らかい商品は圧縮可能なため、以下の係数でサイズを縮小して計算します：
- **ぬいぐるみ (Plush)**: 0.6倍 (全3辺を60%に圧縮)
- **衣類 (Fashion)**: 0.8倍 (高さのみ80%に圧縮)
...
2.  **適合判定**
    - 各配送方法について、サイズ・重量制限に収まるかチェック

3.  **推奨の決定**
    - 適合する配送方法の中から、**最も安い追跡可能オプション**を推奨に設定
    - 追跡可能なオプションがない場合は、最も安いオプションを推奨

4.  **結果の順序**
    - **1番目**: 推奨 (最安の追跡可能オプション)
    - **2番目以降**: 価格順 (追跡不可も含む)
    
    例：追跡可・¥210 が推奨 → 追跡不可・¥160 が2番目 → 追跡可・¥230 が3番目

### 4. 判定理由の生成

#### 推奨プランへの理由付与
推奨された配送方法には、以下の情報が付与されます：
1. **サイズ情報**: 対応サイズ（例：「最大 34×25×3cm」「3辺合計 60cm対応」）
2. **Why Not 理由**: より安い配送方法が**なぜ選ばれなかったか**の説明
...
#### その他の適合プラン
推奨プラン以外で適合する配送方法には、単純なサイズ説明（例：「3辺合計 60cm対応」「最大 34×25×3cm」など）が表示されます。

### 5. UI表示情報
各配送オプションカードには以下の情報が表示されます：
- **サービス名・会社名**
- **追跡可否バッジ**: 「追跡可」（青）/ 「追跡不可」（灰色）
- **最大重量**: 例「〜2kg」
- **追加料金**: 例「+専用BOX代70円別途」（notesから抽出）
- **価格**: 右側に大きく表示
