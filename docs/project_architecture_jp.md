...
*   **仕事**: 商品を箱の中にどう詰めるか（テトリスのような計算）を行います。
*   **仕組み**: `3d-bin-container-packing` というライブラリを使い、総当たりで計算しています。
*   **工夫点 (`calculatePackedResult`)**:
    *   **戦略パターン**: 「体積が大きい順」「底面積が広い順」など、複数の並べ方を試して、一番小さくまとまる結果を採用しています。
    *   **Fallback**: もし計算が失敗しても、単純な体積合計で概算を出す機能が入っています。

**ファイル**: `service/ShippingMatcher.java`
*   **仕事**: 箱のサイズ（3辺合計・重量）を見て、使える配送業者（ヤマト、日本郵便など）を選定します。
*   **工夫点 (`generateWhyNotReason`)**: 単に「使えません」と返すだけでなく、「なぜ安い方の便が使えないのか（例：厚さが1cmオーバーしたから）」という理由を生成しています。これがUIで「〜は使えない理由」として表示されます。
